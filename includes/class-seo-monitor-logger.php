<?php
/**
 * Class for logging events and errors based on WP Logging from Pippin Williamson
 *
 * @package     Seo_Monitor_Logging
 * @copyright   Copyright (c) 2012, Pippin Williamson
 * @license     http://opensource.org/licenses/gpl-2.0.php GNU Public License
*/
class Seo_Monitor_Logging {
	/**
	 * Class constructor.
	 *
	 * @since 1.0
	...
	function __construct() {
		...
		add_action( 'seomonitor_logging_prune_routine', array( $this, 'prune_logs' ) );
	}
	/**
	...
	public function prune_logs(){
		$should_we_prune = apply_filters( 'seomonitor_logging_should_we_prune', false );
	 ...
	 *
	 * @filter seomonitor_logging_force_delete_log         Allows user to override the force delete setting which bypasses the trash
	 */
	public function prune_old_logs( $logs ){
		$force = apply_filters( 'seomonitor_logging_force_delete_log', true );
		foreach( $logs as $l ){
	 ...
	 *
	 * @filter seomonitor_logging_prune_when           Users can change how long ago we are looking for logs to prune
	 * @filter seomonitor_logging_prune_query_args     Gives users access to change any query args for pruning
	 */
	private function get_logs_to_prune(){
		$how_old = apply_filters( 'seomonitor_logging_prune_when', '2 weeks ago' );
		$args = array(
			'post_type'      => 'seo_monitor_log',
			'posts_per_page' => '100',
			'date_query'     => array(
				array(
		...
		$old_logs = get_posts( apply_filters( 'seomonitor_logging_prune_query_args', $args ) );
		return $old_logs;
	} // get_logs_to_prune
	/**
	 * Log types
	...
	*/
	private static function log_types() {
		...
		return apply_filters( 'seomonitor_log_types', $terms );
	}
	/**
	 * Registers the seo_monitor_log Post Type
	 *
	 * @access      public
	 * @since       1.0
	 *
	...
	*/
	public function register_post_type() {
		/* logs post type */
		$log_args = array(
			'labels'          => array( 'name' => __( 'Logs', 'seo-monitor' ) ),
		...
		register_post_type( 'seo_monitor_log', apply_filters( 'seomonitor_logging_post_type_args', $log_args ) );
	}
	/**
	...
	*/
	public function register_taxonomy() {
		register_taxonomy( 'seo_monitor_log_type', 'seo_monitor_log' );
		$types = self::log_types();
		foreach ( $types as $type ) {
			if( ! term_exists( $type, 'seo_monitor_log_type' ) ) {
				wp_insert_term( $type, 'seo_monitor_log_type' );
			}
		}
	}
	/**
	...
	*/
	public static function insert_log( $log_data = array(), $log_meta = array() ) {
		...
		do_action( 'seomonitor_pre_insert_log' );
		// store the log entry
		$log_id = wp_insert_post( $args );
		// set the log type, if any
		if( $log_data['log_type'] && self::valid_type( $log_data['log_type'] ) ) {
			wp_set_object_terms( $log_id, $log_data['log_type'], 'seo_monitor_log_type', false );
		}
		// set log meta, if any
		if( $log_id && ! empty( $log_meta ) ) {
			foreach( (array) $log_meta as $key => $meta ) {
				update_post_meta( $log_id, '_seo_monitor_log_' . sanitize_key( $key ), $meta );
			}
		}
		do_action( 'seomonitor_post_insert_log', $log_id );
		return $log_id;
	}
	/**
	...
	*/
	public static function update_log( $log_data = array(), $log_meta = array() ) {
		do_action( 'seomonitor_pre_update_log', $log_id );
		$defaults = array(
			'post_type'   => 'seo_monitor_log',
			'post_status' => 'publish',
			'post_parent' => 0
		...
		do_action( 'seomonitor_post_update_log', $log_id );
	}
	/**
	...
	*/
	 ...
	 * @return  int
	*/
	public static function get_log_count( $args = array(), $type = null, $meta_query = null ) {
		$defaults = array(
			'post_parent'    => 0,
			'post_type'      => 'seo_monitor_log',
			'posts_per_page' => -1,
...
$GLOBALS['seo_monitor_logs'] = new Seo_Monitor_Logging();